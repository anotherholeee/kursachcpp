...
– `Route(int num, const std::string& vType, const List<std::string>& stops, const std::set<int>& days = {1,2,3,4,5,6,7})` – конструктор с параметрами;
...
– `Time getArrivalTime(const std::string& stop) const` – метод получения времени прибытия на остановку;
...
– `void redo()` – метод повтор команды;
...
## 44. Класс DynamicArray

Данный класс является самописным контейнером - динамическим массивом (аналог std::vector) в системе управления общественным транспортом. Предоставляет функциональность для хранения и управления последовательностью элементов с произвольным доступом.

**Поля:**

– `T* data` – поле для хранения указателя на массив данных;

– `size_t capacity` – поле для хранения емкости массива;

– `size_t size_` – поле для хранения размера массива;

**Внутренние классы:**

– `class Iterator` – класс итератора для обхода элементов массива (random access iterator);
...
– `void push_back(const T& value)` – метод добавления элемента в конец массива (копирование);
...
– `bool empty() const` – метод проверки пустоты массива;

– `void clear()` – метод очистки массива;

– `Iterator begin()` – метод получения итератора на начало массива (неконстантная версия);

– `Iterator end()` – метод получения итератора на конец массива (неконстантная версия);

– `Iterator begin() const` – метод получения итератора на начало массива (константная версия);

– `Iterator end() const` – метод получения итератора на конец массива (константная версия);
