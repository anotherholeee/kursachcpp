...
- `Deque<std::string> allStops` – список всех остановок маршрута;
- `std::set<int> weekDays` – дни недели работы маршрута (1-понедельник, 2-вторник, ..., 7-воскресенье);

**Методы:**

- `Route(int num, const std::string& vType, const Deque<std::string>& stops, const std::set<int>& days = {1,2,3,4,5,6,7})` – конструктор с параметрами;
...
- `const Deque<std::string>& getAllStops() const` – получение списка всех остановок;
...
- `Deque<std::shared_ptr<Trip>> trips` – список рейсов в поездке;
- `Deque<std::string> transferPoints` – точки пересадок;
...
- `Journey(const Deque<std::shared_ptr<Trip>>& tripList, const Deque<std::string>& transfers, Time start, Time end)` – конструктор с параметрами;
...
- `const Deque<std::shared_ptr<Trip>>& getTrips() const` – получение списка рейсов;
- `const Deque<std::string>& getTransferPoints() const` – получение точек пересадок;
...
- `virtual Deque<Journey> findPath(const std::string& start, const std::string& end, const Time& departureTime) = 0` – виртуальный метод поиска пути;
...
- `Deque<Journey> findPath(const std::string& start, const std::string& end, const Time& departureTime) override` – поиск пути;
...
- `Deque<Journey> findPath(const std::string& start, const std::string& end, const Time& departureTime) override` – поиск самого быстрого пути;
- `void execute() override` – выполнение алгоритма;
- `std::string getDescription() const override` – получение описания;
...
- `Deque<Journey> findPath(const std::string& start, const std::string& end, const Time& departureTime) override` – поиск пути с минимальными пересадками;
- `void execute() override` – выполнение алгоритма;
- `std::string getDescription() const override` – получение описания;
...
- `Deque<std::shared_ptr<Route>> findRoutes(const std::string& stopA, const std::string& stopB)` – поиск маршрутов;
- `void execute() override` – выполнение алгоритма;
- `std::string getDescription() const override` – получение описания;
...
- `Deque<std::shared_ptr<Route>> routes` – список маршрутов;
- `Deque<std::shared_ptr<Trip>> trips` – список рейсов;
- `Deque<std::shared_ptr<Vehicle>> vehicles` – список транспортных средств;
- `Deque<std::shared_ptr<Driver>> drivers` – список водителей;
- `Deque<Stop> stops` – массив остановок;
...
- `Deque<std::shared_ptr<Route>> findRoutes(const std::string& stopA, const std::string& stopB)` – поиск маршрутов между остановками;
...
- `const Deque<std::shared_ptr<Trip>>& getTrips() const` – получение списка рейсов;
- `const Deque<std::shared_ptr<Route>>& getRoutes() const` – получение списка маршрутов;
- `const Deque<std::shared_ptr<Vehicle>>& getVehicles() const` – получение списка транспортных средств;
- `const Deque<Stop>& getStops() const` – получение массива остановок;
- `const Deque<std::shared_ptr<Driver>>& getDrivers() const` – получение списка водителей;
...
- `Deque<std::shared_ptr<Trip>> getTripsThroughStop(const std::string& stopName) const` – получение рейсов через остановку;
...
- `Deque<Journey> findJourneysWithTransfers(const std::string& startStop, const std::string& endStop, const Time& departureTime, int maxTransfers = 2) const` – поиск поездок с пересадками;
- `Deque<Journey> findAllJourneysWithTransfers(const std::string& startStop, const std::string& endStop, int maxTransfers = 2) const` – поиск всех поездок с пересадками;
...
- `std::unordered_map<std::shared_ptr<Driver>, Deque<std::shared_ptr<Trip>>> driverTrips` – соответствие водителя и его рейсов;
...
- `Deque<std::shared_ptr<Trip>> getDriverTrips(std::shared_ptr<Driver> driver) const` – получение рейсов водителя;
...
- `Deque<std::unique_ptr<Command>> history` – история команд;
