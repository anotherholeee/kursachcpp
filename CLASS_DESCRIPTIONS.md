...
– static std::shared_ptr<Vehicle> deserialize(const std::string& data) – статический метод десериализации из строки (может выбрасывать ContainerException, так как используется DataManager для создания);
Примечание: Поля type, model, licensePlate объявлены как protected для доступа в наследниках.
...
Примечание: Поля capacity и fuelType объявлены как protected для доступа в наследниках.
...
Примечание: Поля capacity и voltage объявлены как protected для доступа в наследниках.
...
Поля:
– std::string firstName – поле для хранения имени водителя (private);
– std::string lastName – поле для хранения фамилии водителя (private);
– std::string middleName – поле для хранения отчества водителя (private);
– std::string category – поле для хранения категории водительских прав (D, T, и т.д., private);
Методы:
...
Поля:
– int number – поле для хранения номера маршрута (private);
– std::string vehicleType – поле для хранения типа транспортного средства (private);
– std::string startStop – поле для хранения начальной остановки (private);
– std::string endStop – поле для хранения конечной остановки (private);
– List<std::string> allStops – поле для хранения списка всех остановок маршрута (private);
– std::set<int> weekDays – поле для хранения дней недели работы маршрута (1-понедельник, 2-вторник, ..., 7-воскресенье, private);
Методы:
...
– int getStopPosition(const std::string& stop) const – метод получения позиции остановки в маршруте (возвращает позицию остановки (0 - первая остановка) или -1, если не найдена);
– bool isStopBefore(const std::string& stopA, const std::string& stopB) const – метод проверки, находится ли остановка A перед остановкой B (возвращает true, если stopA идет раньше stopB в маршруте);
...
– bool operatesOnDay(int day) const – метод проверки работы маршрута в указанный день (возвращает true, если маршрут работает в этот день недели (1-7));
– std::string serialize() const – метод сериализации в строку;
...
Поля:
– int id – поле для хранения идентификатора остановки (private);
– std::string name – поле для хранения названия остановки (private);
Методы:
...
Данный класс предназначен для представления времени в системе управления общественным транспортом. Хранит часы и минуты, обеспечивает операции сравнения и арифметические операции с временем. Время хранится в формате 24-часового формата (00:00 - 23:59).

Поля:
– int hours – поле для хранения часов (0-23, private);
– int minutes – поле для хранения минут (0-59, private);
Методы:
...
Приватные методы:
– void normalize(int totalMinutes) – метод нормализации времени (приводит время к диапазону 00:00 - 23:59, обрабатывает отрицательные значения и значения больше 1440 минут);
Операторы:
...
– friend std::ostream& operator<<(std::ostream& os, const Time& time) – перегрузка оператора вывода данных класса на экран (дружественная функция);
– friend std::istream& operator>>(std::istream& is, Time& time) – перегрузка оператора ввода данных класса (дружественная функция);
...
Поля:
– int tripId – поле для хранения идентификатора рейса (private);
– std::shared_ptr<Route> route – поле для хранения указателя на маршрут рейса (private);
– std::shared_ptr<Vehicle> vehicle – поле для хранения указателя на транспортное средство (private);
– std::shared_ptr<Driver> driver – поле для хранения указателя на водителя (private);
– Time startTime – поле для хранения времени отправления (private);
– std::map<std::string, Time> schedule – поле для хранения расписания прибытия (остановка -> время прибытия, private);
– int weekDay – поле для хранения дня недели (1-понедельник, 2-вторник, ..., 7-воскресенье, private);
Методы:
...
– Time getEstimatedEndTime() const – метод получения расчетного времени окончания (возвращает время отправления + 60 минут);
– std::string serialize() const – метод сериализации в строку;
...
Поля:
– List<std::shared_ptr<Trip>> trips – поле для хранения списка рейсов в поездке (private);
– List<std::string> transferPoints – поле для хранения точек пересадок (private);
– Time startTime – поле для хранения времени начала поездки (private);
– Time endTime – поле для хранения времени окончания поездки (private);
– int transferCount – поле для хранения количества пересадок (private);
Методы:
...
– int getTotalDuration() const – метод получения общей продолжительности поездки (возвращает продолжительность в минутах);
...
– void display() const – метод вывода информации о поездке (выводит информацию о поездке в консоль);
...
Данный класс является шаблонным базовым классом для алгоритмов в системе управления общественным транспортом. Может принимать либо лямбда-функцию, либо шаблонный параметр (тип). Наследуется от IAlgorithm.

Поля:
– TransportSystem* system – поле для хранения указателя на транспортную систему (protected);
–T algorithmObject – поле для хранения единственного объекта алгоритма (protected);
Внутренние классы:
–Iterator – внутренний класс-итератор для обхода объекта алгоритма (singleton iterator);
Поля класса Iterator:
–T* ptr – поле для хранения указателя на объект алгоритма (private);
– bool visited – поле-флаг, указывающее, был ли элемент посещен (private);
...
Примечание: Итератор является singleton iterator - он предназначен для обхода одного элемента (объекта алгоритма). Итераторы равны, если оба указывают на nullptr или оба посетили элемент.
...
– void execute() override – метод выполнения алгоритма (вызывает algorithmObject(), если он функциональный объект);
– std::string getDescription() const override – метод получения описания алгоритма (возвращает описание по умолчанию, если алгоритм не предоставляет собственного);
...
Данный класс является базовым классом для алгоритмов с TransportSystem в системе управления общественным транспортом. Предназначен для обратной совместимости. Наследуется от IAlgorithm.
...
Поля:
– TransportSystem* system – поле для хранения указателя на транспортную систему (наследуется от BaseAlgorithm, protected);
Методы:
...
Данный класс является шаблонным классом алгоритмов в системе управления общественным транспортом. Принимает только 1 объект, может принимать либо лямбда-функцию, либо шаблонный параметр (тип). Не наследуется от IAlgorithm, является независимым шаблонным классом.

Поля:
– T algorithmObject – поле для хранения единственного объекта алгоритма (private);
Внутренние классы:
...
Поля:
– List<std::unique_ptr<Command>> history – поле для хранения истории команд (private);
– size_t currentIndex – поле для хранения текущего индекса в истории (private, инициализируется значением 0);
– static const size_t MAX_HISTORY_SIZE – константа максимального размера истории (100, private static const);
Методы:
...
Примечание: Метод executeCommand автоматически удаляет все команды после currentIndex при добавлении новой команды, если история была отменена (branching undo).
...
Поля:
– List<std::shared_ptr<Route>> routes – поле для хранения списка маршрутов (private);
– List<std::shared_ptr<Trip>> trips – поле для хранения списка рейсов (private);
– List<std::shared_ptr<Vehicle>> vehicles – поле для хранения списка транспортных средств (private);
– List<std::shared_ptr<Driver>> drivers – поле для хранения списка водителей (private);
– List<Stop> stops – поле для хранения массива остановок (private);
– std::unordered_map<int, std::string> stopIdToName – поле для хранения соответствия ID остановки и её названия (private);
– std::unordered_map<std::string, std::string> adminCredentials – поле для хранения учетных данных администраторов (private);
–JourneyPlanner journeyPlanner – поле для хранения планировщика поездок (private);
–DriverSchedule driverSchedule – поле для хранения расписания водителей (private);
–DataManager dataManager – поле для хранения менеджера данных (private);
–CommandHistory commandHistory – поле для хранения истории команд (private);
– std::unique_ptr<ArrivalTimeCalculationAlgorithm> arrivalTimeAlgorithm – поле для хранения алгоритма расчета времени прибытия (private, Strategy pattern);
– std::unique_ptr<RouteSearchAlgorithm> routeSearchAlgorithm – поле для хранения алгоритма поиска маршрутов (private, Strategy pattern);
Методы:
–TransportSystem() – конструктор по умолчанию;
– bool canUndo() const – метод проверки возможности отмены;
– void undo() – метод отмены действия (может выбрасывать ContainerException, если нет действий для отмены);
– bool canRedo() const – метод проверки возможности повтора;
– void redo() – метод повтор действия (может выбрасывать ContainerException, если нет действий для повтора);
– std::string getLastCommandDescription() const – метод получения описания последней команды;
– std::string getNextCommandDescription() const – метод получения описания следующей команды;
...
– void saveData() – метод сохранения всех данных (может выбрасывать FileException);
– void loadData() – метод загрузки всех данных (может выбрасывать FileException);
...
– void getStopTimetable(int stopId, const Time& startTime, const Time& endTime) – метод получения расписания остановки за период (выводит расписание в консоль);
– void getStopTimetableAll(const std::string& stopName) – метод получения полного расписания остановки (выводит полное расписание в консоль);
– void calculateArrivalTimes(int tripId, double averageSpeed) – метод расчета времени прибытия (может выбрасывать InputException при неверной скорости (<=0), ContainerException при отсутствии рейса или пустом маршруте);
...
– void addRoute(std::shared_ptr<Route> route) – метод добавления маршрута (использует команду, может выбрасывать ContainerException);
– void addTrip(std::shared_ptr<Trip> trip) – метод добавления рейса (использует команду, может выбрасывать ContainerException);
– void addVehicle(std::shared_ptr<Vehicle> vehicle) – метод добавления транспортного средства (использует команду, может выбрасывать ContainerException);
– void addDriver(std::shared_ptr<Driver> driver) – метод добавления водителя (использует команду, может выбрасывать ContainerException);
– void addStop(const Stop& stop) – метод добавления остановки (использует команду, может выбрасывать ContainerException);
– void removeRoute(int routeNumber) – метод удаления маршрута (использует команду, может выбрасывать ContainerException);
– void removeTrip(int tripId) – метод удаления рейса (использует команду, может выбрасывать ContainerException);
– void displayAllRoutes() const – метод вывода всех маршрутов (выводит информацию о всех маршрутах в консоль);
– void displayAllTrips() const – метод вывода всех рейсов (выводит информацию о всех рейсах в консоль);
– void displayAllVehicles() const – метод вывода всех транспортных средств (выводит информацию о всех транспортных средствах в консоль);
– void displayAllStops() const – метод вывода всех остановок (выводит информацию о всех остановках в консоль);
– const List<std::shared_ptr<Trip>>& getTrips() const – метод получения списка рейсов;
...
– List<std::shared_ptr<Trip>> getTripsThroughStop(const std::string& stopName) const – метод получения рейсов через остановку (возвращает список всех рейсов, проходящих через указанную остановку);
– std::string getStopNameById(int id) const – метод получения названия остановки по ID (может выбрасывать ContainerException, если остановка не найдена);
– std::shared_ptr<Route> getRouteByNumber(int number) – метод получения маршрута по номеру (возвращает nullptr, если маршрут не найден);
– std::shared_ptr<Trip> getTripById(int id) – метод получения рейса по ID (возвращает nullptr, если рейс не найден);
– std::shared_ptr<Vehicle> getVehicleByLicensePlate(const std::string& licensePlate) – метод получения транспортного средства по номерному знаку (возвращает nullptr, если транспортное средство не найдено);
–Stop getStopById(int id) – метод получения остановки по ID (может выбрасывать ContainerException, если остановка не найдена);
...
– friend class Trip – класс Trip объявлен дружественным для доступа к приватным членам;

## 36. Класс JourneyPlanner

Данный класс предназначен для планирования поездок в системе управления общественным транспортом. Использует паттерн Facade для упрощения работы с алгоритмами поиска маршрутов. Управляет различными алгоритмами поиска пути. Реализует паттерн Strategy для выбора алгоритма поиска.

Поля:
– TransportSystem* system – поле для хранения указателя на транспортную систему (private);
– std::unique_ptr<BFSAlgorithm> bfsAlgorithm – поле для хранения алгоритма поиска в ширину (private, Strategy pattern);
– std::unique_ptr<FastestPathAlgorithm> fastestAlgorithm – поле для хранения алгоритма поиска самого быстрого пути (private, Strategy pattern);
– std::unique_ptr<MinimalTransfersAlgorithm> minimalTransfersAlgorithm – поле для хранения алгоритма поиска с минимальными пересадками (private, Strategy pattern);
Методы:
...
–Journey findFastestJourney(const std::string& startStop, const std::string& endStop, const Time& departureTime) – метод поиска самой быстрой поездки (может выбрасывать ContainerException, если путь не найден);
–Journey findJourneyWithLeastTransfers(const std::string& startStop, const std::string& endStop, const Time& departureTime) – метод поиска поездки с минимальными пересадками (может выбрасывать ContainerException, если путь не найден);
– void displayJourney(const Journey& journey) const – метод вывода информации о поездке (выводит информацию о поездке в консоль);
...
Поля:
– std::unordered_map<std::shared_ptr<Driver>, List<std::shared_ptr<Trip>>> driverTrips – поле для хранения соответствия водителя и его рейсов (private);
– const int MAX_WORKING_HOURS – константа максимального рабочего времени (12 часов в минутах, private const, значение 12 * 60 = 720);
Методы:
– void assignTripToDriver(std::shared_ptr<Driver> driver, std::shared_ptr<Trip> trip) – метод назначения рейса водителю (добавляет рейс в список рейсов водителя);
– void removeTripFromDriver(std::shared_ptr<Driver> driver, int tripId) – метод удаления рейса у водителя (удаляет рейс из списка рейсов водителя);
– bool isDriverAvailable(std::shared_ptr<Driver> driver, const Time& startTime, const Time& endTime) const – метод проверки доступности водителя (возвращает true, если водитель свободен в указанное время);
– bool checkWorkingHoursCompliance(std::shared_ptr<Driver> driver) const – метод проверки соответствия рабочему времени (возвращает true, если общее время работы не превышает лимит MAX_WORKING_HOURS);
– List<std::shared_ptr<Trip>> getDriverTrips(std::shared_ptr<Driver> driver) const – метод получения рейсов водителя (возвращает список рейсов, назначенных водителю);
– int getTotalWorkingMinutes(std::shared_ptr<Driver> driver) const – метод получения общего рабочего времени водителя (возвращает общее время работы в минутах);
Примечание: Поле MAX_WORKING_HOURS имеет значение 12 * 60 = 720 минут (12 часов).

## 38. Класс DataManager

Данный класс предназначен для управления сохранением и загрузкой данных системы управления общественным транспортом. Обеспечивает персистентность данных транспортной системы. Создает директорию для данных, если она не существует.

Поля:
– std::string dataDirectory – поле для хранения директории для хранения данных (private, по умолчанию "data/");
Методы:
...
Приватные методы:
– void saveStops(TransportSystem& system) – метод сохранения остановок (может выбрасывать FileException);
– void saveVehicles(TransportSystem& system) – метод сохранения транспортных средств (может выбрасывать FileException);
– void saveDrivers(TransportSystem& system) – метод сохранения водителей (может выбрасывать FileException);
– void saveRoutes(TransportSystem& system) – метод сохранения маршрутов (может выбрасывать FileException);
– void saveTrips(TransportSystem& system) – метод сохранения рейсов (может выбрасывать FileException);
– void saveAdminCredentials(TransportSystem& system) – метод сохранения учетных данных администраторов (может выбрасывать FileException);
– void loadStops(TransportSystem& system) – метод загрузки остановок (может выбрасывать FileException);
– void loadVehicles(TransportSystem& system) – метод загрузки транспортных средств (может выбрасывать FileException);
– void loadDrivers(TransportSystem& system) – метод загрузки водителей (может выбрасывать FileException);
– void loadRoutes(TransportSystem& system) – метод загрузки маршрутов (может выбрасывать FileException);
– void loadTrips(TransportSystem& system) – метод загрузки рейсов (может выбрасывать FileException);
– void loadAdminCredentials(TransportSystem& system) – метод загрузки учетных данных администраторов (может выбрасывать FileException);
– int loadVehiclesFromFile(std::ifstream& file, TransportSystem& system, const std::string& fileName) – вспомогательный метод загрузки транспортных средств из файла (возвращает количество загруженных транспортных средств, может выбрасывать FileException);
– int loadTripsFromFile(std::ifstream& file, TransportSystem& system, const std::string& fileName) – вспомогательный метод загрузки рейсов из файла (возвращает количество загруженных рейсов, может выбрасывать FileException);
...
Поля:
– std::string message – поле для хранения сообщения об ошибке (protected);
Методы:
...
Данный класс является самописным контейнером - двусвязным списком (аналог std::list) в системе управления общественным транспортом. Предоставляет функциональность для хранения и управления последовательностью элементов. Является шаблонным классом.

Поля:
– Node* head – поле для хранения указателя на головной узел списка (private);
– Node* tail – поле для хранения указателя на хвостовой узел списка (private);
– size_t size_ – поле для хранения размера списка (private);
Внутренние классы:
– struct Node – структура узла двусвязного списка (private) с полями:
  - T data – данные узла;
  - Node* prev – указатель на предыдущий узел;
  - Node* next – указатель на следующий узел;
  - Конструкторы: Node(const T& value) и Node(T&& value) для копирования и перемещения;
– class Iterator – класс итератора для обхода элементов списка (bidirectional iterator);
Поля класса Iterator:
– Node* node – поле для хранения указателя на текущий узел (private);
Типы класса Iterator (для совместимости со STL):
...
Приватные методы:
– void clearNodes() – метод очистки узлов списка (удаляет все узлы и обнуляет указатели);
– Node* getNodeAt(size_t index) const – метод получения узла по индексу (возвращает nullptr, если индекс выходит за границы);
Дружественные классы:
– friend class List – класс List объявлен дружественным для класса Iterator для доступа к приватным членам (в классе Iterator);